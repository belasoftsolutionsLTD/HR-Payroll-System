...
const STATUS_CFG: Record<ExpenseStatus, { label: string; bg: string; text: string }> = {
  draft:      { label: 'Draft',      bg: 'bg-slate-700',        text: 'text-slate-400'  },
  submitted:  { label: 'Submitted',  bg: 'bg-blue-500/15',      text: 'text-blue-400'   },
  ...
  reimbursed: { label: 'Reimbursed', bg: 'bg-violet-500/10',    text: 'text-violet-400' },
...
function SubmitDrawer({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const Toggle = ({ val, set, label }: { val: boolean; set: (v: boolean) => void; label: string }) => (
    <div className="flex items-center justify-between">
      ...
      <button type="button" onClick={() => set(!val)} className={cn('h-5 w-9 rounded-full relative transition-colors shrink-0', val ? 'bg-indigo-500' : 'bg-slate-700')}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-h-[92vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 shrink-0">
          <h2 className="text-base font-bold text-slate-100">Submit Expense</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {type === 'regular' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Category <span className="text-red-400">*</span></label>
                <select value={category} onChange={e => setCategory(e.target.value)} className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:border-indigo-500">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  ...
                  <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 focus:outline-none focus:border-indigo-500" />
                </div>
                <div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Date <span className="text-red-400">*</span></label>
                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:border-indigo-500" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Description <span className="text-red-400">*</span></label>
                <input value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief description" className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500" />
              </div>
            </>
          )}
          ...
          {type === 'per_diem' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Destination <span className="text-red-400">*</span></label>
                <input value={destination} onChange={e => setDestination(e.target.value)} placeholder="e.g. Mombasa" className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Start Date <span className="text-red-400">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">End Date <span className="text-red-400">*</span></label>
          ...
          {type === 'mileage' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">From <span className="text-red-400">*</span></label>
                  <input value={fromLoc} onChange={e => { setFromLoc(e.target.value); setDistanceKm(''); }} placeholder="Origin address" className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">To <span className="text-red-400">*</span></label>
                  <input value={toLoc} onChange={e => { setToLoc(e.target.value); setDistanceKm(''); }} placeholder="Destination address" className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500" />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Distance (km) <span className="text-red-400">*</span></label>
                ...
                <input type="number" value={distanceKm} onChange={e => setDistanceKm(e.target.value)} placeholder="0" min={0} className="w-full h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 focus:outline-none focus:border-indigo-500" />
              </div>
          ...
          <div>
            ...
            <label className={cn(
              ...
              receipt ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-slate-600 hover:border-indigo-500/50 hover:bg-indigo-500/5'
            )}>
              <Paperclip className={cn('h-4 w-4 shrink-0', receipt ? 'text-emerald-400' : 'text-slate-500')} />
              <div className="flex-1 min-w-0">
                {receipt ? (
                  <p className="text-sm font-semibold text-emerald-400 truncate">{receipt.name}</p>
                ) : (
                  <p className="text-sm text-slate-500">Click to attach PDF or image</p>
                )}
                <p className="text-[11px] text-slate-600 mt-0.5">PDF, PNG, JPG — max 10 MB</p>
              </div>
              {receipt && (
...
function AnalyticsTab() {
  ...
  if (!data) return <div className="py-20 flex justify-center"><div className="h-6 w-6 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin" /></div>;
  ...
  const PieTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="bg-slate-800 border border-slate-600 rounded-xl px-3 py-2 shadow-xl">
        <p className="text-xs text-slate-300 font-semibold capitalize">{d.name}</p>
        <p className="text-sm font-bold text-white">{fmt(d.value)}</p>
        <p className="text-xs text-slate-500">{d.payload.count} claims</p>
      </div>
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <button onClick={() => setYear(y => y - 1)} className="h-8 w-8 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-200 transition-colors"><ChevronLeft className="h-4 w-4" /></button>
          <span className="text-sm font-bold text-slate-100 w-16 text-center">{year}</span>
          <button onClick={() => setYear(y => y + 1)} disabled={year >= now.getFullYear()} className="h-8 w-8 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-200 disabled:opacity-30 transition-colors"><ChevronRight className="h-4 w-4" /></button>
        </div>
        {totalSpend > 0 && (
          <div className="flex items-center gap-1.5 text-sm text-slate-400">
            Total approved: <span className="font-bold text-slate-100">{fmt(totalSpend)}</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl p-5">
        <h3 className="text-sm font-bold text-slate-100 mb-4">Monthly Spend — {year}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {categoryData.length > 0 && (
          <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-slate-100 mb-4">Spend by Category</h3>
        ...
        {deptData.length > 0 && (
          <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-slate-100 mb-4">Spend by Department</h3>
            <ResponsiveContainer width="100%" height={220}>
      ...
      {(data.topSpenders ?? []).length > 0 && (
        <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-700"><h3 className="text-sm font-bold text-slate-100">Top Spenders</h3></div>
          {data.topSpenders.map((s: any, i: number) => {
            const pct = totalSpend > 0 ? Math.round((s.total / totalSpend) * 100) : 0;
            return (
              <div key={i} className="flex items-center gap-4 px-5 py-3 border-b border-slate-700/60 last:border-0">
                <span className="text-sm font-bold text-slate-600 w-5 text-center">{i+1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-200 truncate">{s.employee?.fullName ?? 'Unknown'}</p>
                  <div className="mt-1 h-1 bg-slate-700 rounded-full w-full">
                    <div className="h-full rounded-full" style={{ width: `${pct}%`, background: CHART_COLORS[i % CHART_COLORS.length] }} />
                  </div>
                </div>
                <span className="text-xs text-slate-500 shrink-0">{s.count} claims</span>
                <span className="text-sm font-bold text-slate-100 shrink-0">{fmt(s.total)}</span>
              </div>
            );
          })}
        </div>
...
function DisputeClaimModal({ claim, onClose, onSaved }: { claim: ExpenseClaim; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 space-y-4">
        <div className="flex items-center gap-2">
          <MessageSquareWarning className="h-5 w-5 text-orange-400 shrink-0" />
          <h3 className="text-base font-bold text-slate-100">Dispute Rejection</h3>
        </div>
        <p className="text-xs text-slate-400">Explain why you believe this claim should be reconsidered. HR will review your dispute.</p>
...
function RejectClaimModal({ onClose, onConfirm }: { onClose: () => void; onConfirm: (reason: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 space-y-4">
        <h3 className="text-base font-bold text-slate-100">Reject Expense Claim</h3>
        <textarea
          value={reason}
          onChange={e => setReason(e.target.value)}
          placeholder="Reason for rejection…"
          rows={3}
          className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-red-500 resize-none"
        />
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 text-sm text-slate-400 hover:text-slate-200 transition-colors">Cancel</button>
          <button
            ...
            className="px-5 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white text-sm font-bold disabled:opacity-50 transition-colors"
          >
            Reject
          </button>
        </div>
...
function ClaimDetailDrawer({ claim, isHR, canApprove, onClose, onRefresh, onDisputeClick }: {
  ...
  const stCfg = STATUS_CFG[claim.status] ?? STATUS_CFG.submitted;
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-100">Claim Details</h2>
            <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1.5">
              <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold', tyCfg.bg, tyCfg.text)}>{tyCfg.label}</span>
              <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold', stCfg.bg, stCfg.text)}>{stCfg.label}</span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {claim.employee && (
            <div className="flex items-center gap-3 bg-slate-800 rounded-xl px-4 py-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-100">{claim.employee.fullName}</p>
                <p className="text-xs text-slate-400">{claim.employee.department} · {claim.employee.staffNumber}</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-800/50 rounded-xl px-4 py-2">
            ...
            {claim.rejectionReason && (
              <div className="py-2">
                ...
                <p className="text-xs text-slate-300 mt-1">{claim.rejectionReason}</p>
              </div>
            )}
            {claim.isPolicyViolation && claim.violationReason && (
              <div className="py-2">
                <p className="text-xs text-amber-400 font-semibold">Policy Violation</p>
                <p className="text-xs text-slate-300 mt-1">{claim.violationReason}</p>
              </div>
            )}
          </div>
...
function EditClaimDrawer({ claim, onClose, onSaved }: { claim: ExpenseClaim; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 shrink-0">
          <h2 className="text-base font-bold text-slate-100">Edit Claim</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors"><X className="h-5 w-5" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Amount ({claim.currency})</label>
          ...
          <p className="text-[11px] text-slate-600">Only amount, date, category, description and notes can be changed. Other fields are locked after submission.</p>
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-700 shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-sm text-slate-400 hover:text-slate-200 transition-colors">Cancel</button>
      ...
      <div className="border-b border-slate-700/60 bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
            <div>
              <h1 className="text-xl font-black text-slate-100 tracking-tight">Expenses</h1>
              <p className="text-xs text-slate-400 mt-0.5">Submit and manage expense claims</p>
            </div>
            <div className="flex items-center gap-2">
              {isHR && <button onClick={() => downloadFile(`${API_BASE_URL}/expense-claims/export`, 'expenses-export.csv').catch(err => alert(err.message))} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-700 bg-slate-800 text-slate-400 hover:text-slate-200 text-sm font-semibold transition-colors"><Download className="h-4 w-4" /> Export</button>}
              <button onClick={() => setSubmitOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition-colors"><Plus className="h-4 w-4" /> Submit Expense</button>
            </div>
          </div>
          {/* Stats (HR only) */}
          {stats && isHR && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              {[
                ...
                { label: 'Policy Violations', value: String(stats.violations), sub: '', color: stats.violations > 0 ? 'text-red-400' : 'text-slate-400' },
              ].map(({ label, value, sub, color }) => (
                <div key={label} className="bg-[#1e293b] border border-slate-700/60 rounded-xl px-4 py-3">
            ...
            {tabs.map(({ key, label }) => (
              <button key={key} onClick={() => setTab(key)}
                className={cn('px-4 py-2 rounded-t-lg text-sm font-semibold whitespace-nowrap transition-all border-b-2',
                  tab === key ? 'text-indigo-300 border-indigo-500 bg-slate-900/60' : 'text-slate-500 border-transparent hover:text-slate-300 hover:bg-slate-800/40')}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="relative flex-1 min-w-48">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search expenses…" className="w-full h-9 pl-9 pr-3 bg-slate-800 border border-slate-700 rounded-xl text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500" />
              </div>
              <select value={status} onChange={e => setStatus(e.target.value)} className="h-9 bg-slate-800 border border-slate-700 rounded-xl px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500">
                <option value="">All Statuses</option>
                {Object.entries(STATUS_CFG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
              {(search || status) && <button onClick={() => { setSearch(''); setStatus(''); }} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 px-2 py-1 rounded-lg hover:bg-slate-800 transition-colors"><X className="h-3.5 w-3.5" /> Clear</button>}
              <span className="text-xs text-slate-600 ml-auto self-center">{filtered.length} result{filtered.length !== 1 ? 's' : ''}</span>
            </div>

            {/* Table */}
            <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl overflow-hidden">
              <div className="grid border-b border-slate-700 bg-slate-800/60" style={{ gridTemplateColumns: '80px 1fr 100px 120px 100px 120px 80px 140px' }}>
              ...
              {loading ? (
                ...
                <div className="py-16 text-center">
                  <p className="text-slate-400 text-sm font-semibold">No expenses found</p>
                  <button onClick={() => setSubmitOpen(true)} className="mt-3 text-indigo-400 text-xs underline">Submit your first expense</button>
                </div>
              ) : filtered.map(c => {
                const stCfg = STATUS_CFG[c.status] ?? STATUS_CFG.submitted;
                ...
                return (
                  <div key={c._id} style={{ gridTemplateColumns: '80px 1fr 100px 120px 100px 120px 80px 140px' }}
                    ...
                    <div className="px-3 py-3 text-xs text-slate-400">{fmtDate(c.date)}</div>
                    ...
                    <div className="px-3 py-3 text-xs text-slate-400 capitalize">{c.category ?? '—'}</div>
                    <div className="px-3 py-3 text-sm font-bold text-slate-100">{fmt(c.amount, c.currency)}</div>
                    <div className="px-3 py-3 text-xs text-slate-400 truncate">{c.employee?.fullName ?? 'You'}</div>
                    <div className="px-3 py-3"><span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full', stCfg.bg, stCfg.text)}>{stCfg.label}</span></div>
                    <div className="px-3 py-3 flex items-center gap-1 flex-wrap">
                      {/* View */}
                      <button title="View details" onClick={() => setDetailClaim(c)}
                        className="h-7 w-7 rounded-lg bg-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-slate-600 transition-colors">
