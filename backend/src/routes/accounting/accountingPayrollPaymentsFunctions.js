...
const markPayrollCyclePaid = async (req, res) => {
  ...
  const closeEntry = await findOne('gl_journal_entries', { referenceId: cycle._id, referenceModel: 'payroll_cycles', source: 'payroll_cycle_close', status: 'posted' });
  if (!closeEntry) return returnFunction(res, 400, false, 'No posted payroll close entry was found for this cycle — nothing to pay.');
  const alreadyPaid = await findOne('gl_journal_entries', { referenceId: cycle._id, referenceModel: 'payroll_cycles', source: 'payroll_payment', status: 'posted' });
  ...
  const payload = {
    date: new Date(), description: `Payroll payment — cycle ${cycle._id} (${req.body.paymentReference})`, source: 'payroll_payment', sourceModule: 'accounting',
  ...
  try {
    ...
    await logPostingFailure({ source: 'payroll_payment', sourceModule: 'accounting', referenceId: cycle._id, referenceModel: 'payroll_cycles', attemptedPayload: payload, error: err });
