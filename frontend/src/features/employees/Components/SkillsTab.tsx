...
function SkillsSection({ employeeId, skills, isHR, onChanged }: { employeeId: string; skills: string[]; isHR: boolean; onChanged: () => void }) {
  ...
  const addSkill = (skill: string) => save([...skills, skill]);
    ...
    <div className="rounded-xl border bg-white p-4">
      ...
      {isHR && (
        <SkillPicker
          ...
          saving={saving}
          onAdd={addSkill}
          selectClassName="h-9 px-3 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
          inputClassName="flex-1 h-9 px-3 border border-brand-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
      ...
      {showForm && (
        <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-3 space-y-2 mb-3">
          <div className="grid grid-cols-2 gap-2">
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Certification name *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <input value={form.issuingOrganization} onChange={e => setForm(f => ({ ...f, issuingOrganization: e.target.value }))} placeholder="Issuing organization *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <input type="date" value={form.issueDate} onChange={e => setForm(f => ({ ...f, issueDate: e.target.value }))} className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <input type="date" value={form.expiryDate} onChange={e => setForm(f => ({ ...f, expiryDate: e.target.value }))} placeholder="Expiry (optional)" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
          </div>
        ...
        <div className="divide-y divide-gray-100">
          {certifications.map(c => (
            <div key={c.id} className="flex items-center justify-between py-2.5">
              ...
              {isHR && (
                <button onClick={() => remove(c.id)} className="h-7 w-7 rounded-lg flex items-center justify-center text-brand-text-secondary hover:text-red-500 hover:bg-red-50 transition-colors">
  ...
  return (
    <div className="rounded-xl border bg-white p-4">
      ...
      {showForm && (
        <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-3 space-y-2 mb-3">
          <div className="grid grid-cols-2 gap-2">
            <input value={form.institution} onChange={e => setForm(f => ({ ...f, institution: e.target.value }))} placeholder="Institution *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <input value={form.degree} onChange={e => setForm(f => ({ ...f, degree: e.target.value }))} placeholder="Degree *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <input value={form.fieldOfStudy} onChange={e => setForm(f => ({ ...f, fieldOfStudy: e.target.value }))} placeholder="Field of study *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            <div className="grid grid-cols-2 gap-2">
              <input type="number" value={form.startYear} onChange={e => setForm(f => ({ ...f, startYear: e.target.value }))} placeholder="Start year *" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
              <input type="number" value={form.endYear} onChange={e => setForm(f => ({ ...f, endYear: e.target.value }))} placeholder="End year" className="h-9 px-3 border border-brand-border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-primary/20" />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={add} disabled={saving} className="flex items-center gap-1.5 h-8 px-3 bg-brand-primary hover:bg-brand-primary-hover text-white text-xs font-semibold rounded-lg disabled:opacity-50">
        ...
        <div className="divide-y divide-gray-100">
          {educationHistory.map(e => (
            <div key={e.id} className="flex items-center justify-between py-2.5">
              ...
              {isHR && (
                <button onClick={() => remove(e.id)} className="h-7 w-7 rounded-lg flex items-center justify-center text-brand-text-secondary hover:text-red-500 hover:bg-red-50 transition-colors">
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              )}
            </div>
