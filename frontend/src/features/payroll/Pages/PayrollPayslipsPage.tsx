    ...
    <div className="min-h-screen bg-[#0f172a]">
      <div className="border-b border-slate-700/60 bg-slate-900/50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5">
          <h1 className="text-xl font-black text-slate-100 tracking-tight">My Payslips</h1>
          <p className="text-xs text-slate-400 mt-0.5">Your complete payslip history</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-3">
        {loading ? (
          ...
          <div className="py-20 text-center text-slate-600 text-sm">No payslips yet. Your payslips will appear here after each payroll cycle is closed.</div>
        ) : payslips.map(slip => {
          const isExp = expanded === slip._id;
          return (
            <div key={slip._id} className="bg-[#1e293b] border border-slate-700/60 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-100">{MONTHS[slip.period.month - 1]} {slip.period.year}</p>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-semibold text-slate-300">{fmt(slip.grossPay)}</p>
                  </div>
                  <div className="text-right">
                  ...
                  <button onClick={() => downloadFile(`${API_BASE_URL}/payroll/payslips/${slip._id}/pdf`, `payslip-${slip._id}.pdf`).catch(err => alert(err.message))}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-colors" title="Download PDF">
                    <Download className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => setExpanded(isExp ? null : slip._id)}
                    className="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-slate-700 transition-colors">
                    {isExp ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </button>
                </div>
              </div>
              {isExp && (
                <div className="border-t border-slate-700/60 px-5 py-4 text-xs text-slate-500 text-center">
