    ...
    <div className="min-h-screen bg-white">
      <div className="border-b border-brand-border/60 bg-white/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-black text-brand-text tracking-tight">Payroll Analytics</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
        {loading ? (
          ...
          <div className="py-20 text-center text-brand-text-muted text-sm">No closed payroll runs yet — analytics will appear once a cycle is closed.</div>
        ) : (
          <>
                ...
                {data.topEarners.map((e, i) => (
                  <div key={e.employeeId} className="flex items-center justify-between py-2.5">
                    <div className="flex items-center gap-3">
                      <span className={`h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold ${i < 3 ? 'bg-amber-500/20 text-amber-400' : 'bg-brand-bg-soft text-brand-text-muted'}`}>
                        {i < 3 ? <Trophy className="h-3 w-3" /> : i + 1}
                      </span>
                      <div>
                        <p className="text-sm font-medium text-brand-text">{e.fullName}</p>
                        <p className="text-[10px] text-brand-text-muted">{e.department} · {e.staffNumber}</p>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-brand-text">{fmt(e.netPay)}</span>
                  </div>
                ))}
                {data.topEarners.length === 0 && <p className="text-sm text-brand-text-muted text-center py-6">No data.</p>}
              </div>
            </ChartCard>
          </>
        )}
