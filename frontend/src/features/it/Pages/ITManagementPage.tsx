...
function StatCard({ label, value, color = '#6366f1', icon: Icon }: {
  ...
  return (
    <Card>
      ...
      <div className="text-[24px] font-black text-slate-100">{value}</div>
    </Card>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-lg rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-[15px] font-bold text-slate-100">Add Device</h2>
        ...
        <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Condition*</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Purchase Date</label>
              <input type="date" value={form.purchaseDate} onChange={e => set('purchaseDate', e.target.value)}
                className="w-full h-9 px-3 rounded-lg bg-slate-700 text-slate-200 text-[13px] border border-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Warranty Expiry</label>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={2}
              className="w-full px-3 py-2 rounded-lg bg-slate-700 text-slate-200 text-[13px] border border-slate-600 placeholder-slate-500 resize-none focus:outline-none focus:ring-1 focus:ring-indigo-500" />
          </div>
        </div>
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-slate-700">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-[13px] text-slate-400 hover:bg-slate-700 transition-colors">Cancel</button>
          <button onClick={save} disabled={saving || !form.name || !form.serialNumber}
            className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[13px] font-semibold disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-[15px] font-bold text-slate-100">Add Software / App</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
          {[
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">License Type</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Total Licenses</label>
              <input type="number" value={form.totalLicenses} onChange={e => set('totalLicenses', e.target.value)}
                className="w-full h-9 px-3 rounded-lg bg-slate-700 text-slate-200 text-[13px] border border-slate-600 focus:outline-none" />
            </div>
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Cost / License</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Renewal Date</label>
...
function AddRequestModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-[15px] font-bold text-slate-100">New IT Request</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5 space-y-3 max-h-[75vh] overflow-y-auto">
          <div>
            <label className="text-[11px] text-slate-400 block mb-1">Request Type</label>
          ...
          {isRepair && (
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Which device needs repair?</label>
              {selectedDevice ? (
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/30">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">{selectedDevice.serialNumber}</p>
                  </div>
                  <button onClick={() => setSelectedDevice(null)} className="text-slate-500 hover:text-slate-300 shrink-0">
                ...
                <>
                  <div className="relative mb-1.5">
                    ...
                    <input value={deviceSearch} onChange={e => setDeviceSearch(e.target.value)} placeholder="Search devices by name or serial…"
                      className="w-full h-9 pl-8 pr-3 rounded-lg bg-slate-700 text-slate-200 text-[13px] border border-slate-600 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                  </div>
                  <div className="max-h-36 overflow-y-auto rounded-lg border border-slate-700 divide-y divide-slate-700/50">
                    {filteredDevices.length === 0
                      ? <p className="text-center py-4 text-slate-500 text-[12px]">No devices found</p>
                      : filteredDevices.map(d => {
                          ...
                          return (
                            <button key={d._id} onClick={() => setSelectedDevice(d)}
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-[13px] text-slate-200 truncate">{d.name}</p>
                                <p className="text-[11px] text-slate-500">{d.serialNumber}</p>
                              </div>
                              {d.assignedEmployee && (
                                <span className="text-[10px] text-slate-500 truncate max-w-[80px]">{d.assignedEmployee.fullName}</span>
                              )}
                            </button>
                          );
                        })}
            ...
            <div className="flex gap-2">
              {['low', 'medium', 'high'].map(p => (
                <button key={p} onClick={() => set('priority', p)}
                  className={`flex-1 py-2 rounded-lg text-[12px] font-semibold capitalize transition-colors ${form.priority === p ? 'ring-2 ring-indigo-500' : ''}`}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <div>
            <h2 className="text-[15px] font-bold text-slate-100">Assign Device</h2>
            <p className="text-[12px] text-slate-400 mt-0.5">{device.name}</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5">
          ...
          <div className="max-h-56 overflow-y-auto rounded-lg border border-slate-700 divide-y divide-slate-700/50">
            {filtered.length === 0 ? (
              <p className="text-center py-6 text-slate-500 text-[13px]">No employees found</p>
            ) : filtered.map(e => (
              <button key={e._id} onClick={() => setSelected(e)}
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${
                  selected?._id === e._id ? 'bg-indigo-600/20' : 'hover:bg-slate-700/40'
                }`}>
                <div className="h-8 w-8 rounded-full bg-indigo-600/30 flex items-center justify-center text-[12px] font-bold text-indigo-300 shrink-0">
                  {e.fullName[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-semibold text-slate-200 truncate">{e.fullName}</div>
                  {e.designation && <div className="text-[11px] text-slate-500 truncate">{e.designation}</div>}
                </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={fetchAll} className="h-9 w-9 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-800 hover:text-slate-200 transition-colors">
      ...
      <div className="rounded-2xl overflow-hidden border border-slate-700/60 bg-[#1e293b]">
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {devices.length === 0 ? (
              <tr><td colSpan={isHR ? 8 : 7} className="text-center py-10 text-slate-500">No devices found</td></tr>
            ) : devices.map(d => {
              const Icon = DEVICE_ICONS[d.type] || Package;
                ...
                <tr key={d._id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-8 w-8 rounded-lg flex items-center justify-center bg-indigo-600/20">
                        <Icon className="h-4 w-4 text-indigo-400" />
                      </div>
                      <div>
                        <div className="font-semibold text-slate-100">{d.name}</div>
                        {d.brand && <div className="text-[11px] text-slate-500">{d.brand} {d.model || ''}</div>}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-400 capitalize">{d.type}</td>
                  <td className="px-4 py-3 text-slate-400 font-mono text-[12px]">{d.serialNumber}</td>
                  <td className="px-4 py-3">
                    {d.assignedEmployee ? (
                      <div className="flex items-center gap-1.5">
                        <div className="h-5 w-5 rounded-full bg-indigo-600/40 flex items-center justify-center text-[9px] font-bold text-indigo-300">
                          {d.assignedEmployee.fullName[0]}
                        </div>
                        <span className="text-slate-300">{d.assignedEmployee.fullName}</span>
                      </div>
                    ) : (
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[15px] font-bold text-slate-100">Grant Access</h2>
              <p className="text-[11px] text-slate-400">{app.name} · {seatsLeft === Infinity ? '∞' : seatsLeft} seats left</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200"><X className="h-4 w-4" /></button>
        </div>
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <span className="text-[13px] text-slate-400">{apps.length} applications</span>
        {isHR && (
          <button onClick={() => setShowAdd(true)}
            className="flex items-center gap-1.5 h-9 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[13px] font-semibold transition-colors">
      ...
      <div className="grid grid-cols-3 gap-3">
        {apps.map(app => {
          ...
          return (
            <Card key={app._id}>
              ...
              <div className="font-bold text-slate-100 text-[14px] mb-0.5">{app.name}</div>
              {app.vendor && <div className="text-[11px] text-slate-500 mb-2">{app.vendor}</div>}
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[11px] text-slate-400 whitespace-nowrap">{app.assignedLicenses}/{app.totalLicenses || '∞'}</span>
              </div>
        ...
        {apps.length === 0 && (
          <div className="col-span-3 text-center py-12 text-slate-500">No software apps added yet</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <div>
            <h2 className="text-[15px] font-bold text-slate-100">{isRepair ? 'Assign Technician' : 'Assign Request'}</h2>
            <p className="text-[12px] text-slate-400 mt-0.5 truncate max-w-xs">{request.subject}</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 shrink-0"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5 space-y-3">
          ...
          {isRepair && request.device && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-[12px]">
              ...
              <div>
                ...
                <span className="text-slate-500 ml-2">will be marked <span className="text-amber-400 font-semibold">In Repair</span> once assigned</span>
              </div>
            </div>
          )}
          {isRepair && !request.device && request.deviceName && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-slate-700/50 border border-slate-600 text-[12px]">
              <Wrench className="h-4 w-4 text-slate-400 shrink-0" />
              <span className="text-slate-300">{request.deviceName}</span>
            </div>
          )}
          ...
          <div>
            ...
            <div className="max-h-44 overflow-y-auto rounded-lg border border-slate-700 divide-y divide-slate-700/50">
              {filtered.length === 0 ? (
                <p className="text-center py-6 text-slate-500 text-[13px]">No employees found</p>
              ) : filtered.map(e => (
                <button key={e._id} onClick={() => setSelected(e)}
                  ...
                  <div className="h-7 w-7 rounded-full bg-indigo-600/30 flex items-center justify-center text-[11px] font-bold text-indigo-300 shrink-0">
                    {e.fullName[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-slate-200 truncate">{e.fullName}</div>
          ...
          {isRepair && (
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Repair Instructions / Notes (optional)</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.85)' }}>
      <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ background: '#1e293b' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-[15px] font-bold text-slate-100">Mark as Resolved</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5 space-y-3">
          {isRepair && request.device && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-[12px]">
              ...
              <span className="text-slate-300"><span className="text-emerald-300 font-semibold">{request.device.name}</span> will be returned to <span className="text-emerald-400 font-semibold">Available</span></span>
            </div>
          )}
          {isRepair && (
            <div>
              <label className="text-[11px] text-slate-400 block mb-1">Device condition after repair</label>
              <div className="flex gap-2">
                {(['new', 'good', 'fair', 'needs_repair'] as const).map(c => (
                  <button key={c} onClick={() => setUpdatedCondition(c)}
                    className={`flex-1 py-1.5 rounded-lg text-[11px] font-semibold capitalize transition-all border ${
                      updatedCondition === c ? 'border-indigo-500 bg-indigo-500/20 text-indigo-300' : 'border-slate-600 bg-slate-700/50 text-slate-400'
                    }`}>
                    {c.replace('_', ' ')}
                  </button>
                ))}
...
function RequestsTab() {
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <span className="text-[13px] text-slate-400">{requests.length} requests</span>
        <button onClick={() => setShowAdd(true)}
          className="flex items-center gap-1.5 h-9 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[13px] font-semibold transition-colors">
      ...
      <div className="rounded-2xl overflow-hidden border border-slate-700/60 bg-[#1e293b]">
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {requests.length === 0 ? (
              <tr><td colSpan={isHR ? 9 : 8} className="text-center py-10 text-slate-500">No IT requests yet</td></tr>
            ) : requests.map(r => {
              ...
              return (
                <tr key={r._id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    {r.repairNotes && <div className="text-[11px] text-slate-500 mt-0.5 truncate max-w-[160px]">{r.repairNotes}</div>}
                  </td>
                  <td className="px-4 py-3 text-slate-400 capitalize whitespace-nowrap">{r.type.replace(/_/g, ' ')}</td>
                  <td className="px-4 py-3">
                    {isRepair && r.device ? (
                      <div className="flex items-center gap-1.5">
                        <Wrench className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                        <div>
                          <div className="text-[12px] text-slate-300 font-semibold">{r.device.name}</div>
                          <div className="text-[10px] text-slate-500 capitalize">{r.device.status.replace('_', ' ')}</div>
                        </div>
                      </div>
                    ) : isRepair && r.deviceName ? (
                      <span className="text-[12px] text-slate-400">{r.deviceName}</span>
                    ) : (
                      <span className="text-slate-600">—</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{r.requesterName}</td>
                  <td className="px-4 py-3">
                    {r.assigneeName ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-slate-300">{r.assigneeName}</span>
                      </div>
                    ) : (
  ...
  return (
    <div className="min-h-full" style={{ background: '#0f172a' }}>
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-slate-100">Assets Management</h1>
          <p className="text-[13px] text-slate-400 mt-0.5">Devices, software licenses, and IT support</p>
        </div>
      </div>
