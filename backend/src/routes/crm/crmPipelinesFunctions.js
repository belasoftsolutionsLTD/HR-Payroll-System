...
const listPipelines = async (req, res) => {
  ...
  const pipelines = await findMany('crm_pipelines', { isActive: { $ne: false } }, { sort: { isDefault: -1, name: 1 } });
...
const createPipeline = async (req, res) => {
  ...
  const existingCount = await countDocuments('crm_pipelines', { isActive: { $ne: false } });
  const doc = {
    name: req.body.name.trim(),
    stages: normalizeStages(rawStages),
    isDefault: existingCount === 0,
    isActive: true,
    createdAt: new Date(),
    updatedAt: new Date(),
  };
  const result = await insertOne('crm_pipelines', doc);
...
const updatePipeline = async (req, res) => {
  const level = await getCrmAccessLevel(req.user);
  if (level !== 'admin') return returnFunction(res, 403, false, 'Not authorized.');
  const pipeline = await findOne('crm_pipelines', { _id: new ObjectId(req.params.id) });
  ...
  if (Array.isArray(req.body.stages) && req.body.stages.length) update.stages = normalizeStages(req.body.stages);
  await updateOne('crm_pipelines', { _id: pipeline._id }, { $set: update });
...
const deletePipeline = async (req, res) => {
  const level = await getCrmAccessLevel(req.user);
  if (level !== 'admin') return returnFunction(res, 403, false, 'Not authorized.');
  const inUse = await countDocuments('crm_deals', { pipelineId: new ObjectId(req.params.id) });
  if (inUse > 0) return returnFunction(res, 400, false, `${inUse} deal(s) still use this pipeline — move or close them first.`);
  await updateOne('crm_pipelines', { _id: new ObjectId(req.params.id) }, { $set: { isActive: false, updatedAt: new Date() } });
