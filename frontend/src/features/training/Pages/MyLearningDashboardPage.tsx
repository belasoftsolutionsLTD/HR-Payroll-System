...
function StatTile({ icon: Icon, label, value, accent }: { icon: any; label: string; value: number; accent?: string }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
      <div className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${accent || 'bg-brand-primary/10 text-brand-primary'}`}>
...
export function MyLearningDashboardPage({ locale }: { locale: string }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-brand-text">My Training</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href={`/${locale}/my/training/catalog`} className="text-sm text-brand-primary hover:underline">Browse Catalog</Link>
        </div>
        <div className="space-y-3">
          {enrollments.filter((e) => e.courseId && e.status !== 'waived').map((e) => (
            <Link key={e._id} href={`/${locale}/my/training/courses/${e.courseId}/learn`} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-brand-primary/40 hover:bg-slate-50 transition">
              <div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-28 h-1.5 bg-slate-100 rounded-full overflow-hidden hidden sm:block">
                  <div className="h-full bg-brand-primary rounded-full" style={{ width: `${e.progressPercentage}%` }} />
                </div>
      ...
      {paths.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div className="space-y-3">
            {paths.map((p) => (
              <Link key={p._id} href={`/${locale}/my/training/learning-paths/${p.learningPathId}`} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-brand-primary/40 hover:bg-slate-50 transition">
                <div>
                ...
                <div className="w-28 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-primary rounded-full" style={{ width: `${p.progressPercentage}%` }} />
                </div>
              </Link>
            ))}
          </div>
