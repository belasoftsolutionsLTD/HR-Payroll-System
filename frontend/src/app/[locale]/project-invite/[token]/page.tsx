...
import { CheckCircle2, XCircle, Users, Calendar, AlertTriangle } from 'lucide-react';
...
export default function ProjectInviteResponsePage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/public/project-invites/${token}`)
      .then((r) => r.json())
      .then((d) => {
        if (!d.success) { setErrorMsg(d.message || 'This invite link is invalid or has expired.'); setView('error'); return; }
        setInvite(d.data);
        setView(d.data.status === 'pending' ? 'ready' : 'done');
      })
      .catch(() => { setErrorMsg('Something went wrong loading your invite.'); setView('error'); });
  }, [token]);

  const respond = (status: 'accepted' | 'declined') => {
    setSubmitting(true);
    fetch(`${API_BASE}/public/project-invites/${token}/respond`, {
      method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-brand-bg-soft flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        {view === 'loading' && (
          <p className="text-center text-brand-text-secondary text-sm">Loading your invitation…</p>
        )}
        ...
        {view === 'error' && (
          <div className="text-center space-y-3">
            ...
            <p className="text-xs text-brand-text-muted">Please contact whoever invited you if you believe this is a mistake.</p>
          </div>
        )}

        {invite && (view === 'ready' || view === 'confirm') && (
          <div className="space-y-5">
            <div>
              <h1 className="text-xl font-bold text-brand-text">Project Invitation</h1>
              <p className="text-sm text-brand-text-secondary mt-1">Hi {invite.name}, {invite.invitedByName} has invited you to join a project.</p>
            </div>
            <div className="space-y-3 bg-brand-bg-soft rounded-xl p-4">
              <div className="flex items-center gap-2 text-sm">
                <Users className="h-4 w-4 text-brand-primary shrink-0" />
                <span className="text-brand-text">{invite.projectName} · {invite.projectRole === 'team_leader' ? 'Team Leader' : 'Member'}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Calendar className="h-4 w-4 text-brand-primary shrink-0" />
                <span className="text-brand-text">Access through {new Date(invite.contractEndDate).toLocaleDateString('en-KE', { dateStyle: 'long' })}</span>
              </div>
              <p className="text-xs text-brand-text-muted">This invite link expires on {new Date(invite.expiresAt).toLocaleDateString('en-KE', { dateStyle: 'long' })}.</p>
            </div>
            ...
            {view === 'confirm' && decision && (
              <div className="space-y-3 border-t border-brand-border pt-4">
                <p className="text-sm text-brand-text text-center">
                  Are you sure you want to <strong>{decision === 'accepted' ? 'accept' : 'decline'}</strong> this invite? This cannot be undone.
                </p>
                <div className="flex gap-3">
                  <button
          ...
          <div className="text-center space-y-3">
            {invite.status === 'accepted' ? (
              <>
                ...
                <p className="text-brand-text font-semibold">You've accepted this invite.</p>
                <p className="text-sm text-brand-text-secondary">Check your email — your login credentials have been sent to you.</p>
              </>
            ) : invite.status === 'declined' ? (
              <>
                <XCircle className="h-10 w-10 text-brand-text-muted mx-auto" />
                <p className="text-brand-text font-semibold">You've declined this invite.</p>
              </>
            ) : (
              <>
                <AlertTriangle className="h-10 w-10 text-brand-danger mx-auto" />
                <p className="text-brand-text font-semibold">This invite is no longer available.</p>
              </>
            )}
          </div>
        )}
