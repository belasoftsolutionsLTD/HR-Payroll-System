...
function Input({ value, onChange, placeholder, type = 'text', disabled = false }: {
  ...
  return (
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      disabled={disabled}
      className="w-full h-10 px-3 rounded-lg border border-slate-700 bg-slate-800 text-slate-200 text-[13px] placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed" />
  );
}
...
function Select({ value, onChange, children }: { value: string; onChange: (v: string) => void; children: React.ReactNode }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      className="w-full h-10 px-3 rounded-lg border border-slate-700 bg-slate-800 text-slate-200 text-[13px] focus:outline-none focus:ring-1 focus:ring-indigo-500">
      {children}
    </select>
  );
}
...
function SaveButton({ onClick, saving }: { onClick: () => void; saving: boolean }) {
  return (
    <button onClick={onClick} disabled={saving}
      className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[13px] font-semibold transition-colors disabled:opacity-50">
...
function SectionHeader({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="mb-5">
      <h2 className="text-[17px] font-bold text-slate-100">{title}</h2>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="text-left px-4 py-3 text-[11px] text-slate-400 uppercase tracking-wider font-semibold">Permission</th>
              {ROLES.map(r => (
                <th key={r} className="text-center px-4 py-3 text-[11px] text-slate-400 uppercase tracking-wider font-semibold">
                  {ROLE_LABELS[r]}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {PERMISSIONS.map(p => (
              <tr key={p.key} className="border-b border-slate-700/50 hover:bg-slate-700/20">
                ...
                {ROLES.map(r => (
                  <td key={r} className="px-4 py-3 text-center">
                    {p.allowedRoles.includes(r) ? (
                      ...
                      <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-slate-700">
...
function EmailTemplatesSection() {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {EMAIL_TEMPLATES.map(t => (
          <button key={t.id} onClick={() => open(t.id)}
            className={`text-left p-4 rounded-xl border transition-colors ${active === t.id ? 'border-indigo-500 bg-indigo-600/10' : 'border-slate-700/60 bg-[#1e293b] hover:border-slate-600'}`}>
            <div className="flex items-center justify-between">
              <span className="font-semibold text-slate-200 text-[13px]">{t.label}</span>
              <ChevronRight className={`h-4 w-4 text-slate-500 transition-transform ${active === t.id ? 'rotate-90' : ''}`} />
            </div>
      ...
      {active && (
        <div className="rounded-2xl border border-slate-700/60 overflow-hidden bg-[#1e293b]">
          ...
          <div className="p-4 space-y-3">
            ...
            <Field label="Body">
              <textarea value={body} onChange={e => setBody(e.target.value)} rows={8}
                className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-800 text-slate-200 text-[13px] font-mono resize-none focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </Field>
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {/* Password policy */}
        <div className="p-4 rounded-2xl border border-slate-700/60 bg-[#1e293b]">
          <h3 className="text-[14px] font-bold text-slate-200 mb-3">Password Policy</h3>
          ...
          <div className="mt-3 space-y-2">
            {[
              ...
              <label key={rule} className="flex items-center gap-2.5 cursor-pointer">
                <div className="h-4 w-4 rounded border border-indigo-500 bg-indigo-600/30 flex items-center justify-center">
                  <Check className="h-3 w-3 text-indigo-400" />
                </div>
                <span className="text-[13px] text-slate-300">{rule}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl border border-slate-700/60 bg-[#1e293b]">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-[14px] font-bold text-slate-200">Two-Factor Authentication</h3>
              <p className="text-[12px] text-slate-500 mt-0.5">Require 2FA for admin and HR roles</p>
            </div>
            <button onClick={() => setTwoFaEnabled(v => !v)}
              className={`h-6 w-11 rounded-full transition-colors relative ${twoFaEnabled ? 'bg-indigo-600' : 'bg-slate-700'}`}>
        ...
        {/* API Key */}
        <div className="p-4 rounded-2xl border border-slate-700/60 bg-[#1e293b]">
          <h3 className="text-[14px] font-bold text-slate-200 mb-2">API Key</h3>
          <p className="text-[12px] text-slate-500 mb-3">Used for external integrations and webhooks</p>
          <div className="flex items-center gap-2">
            <div className="flex-1 h-10 px-3 rounded-lg border border-slate-700 bg-slate-900 flex items-center">
              <span className="text-[13px] font-mono text-slate-400">
                {showKey ? 'sk-live-abc123def456ghi789jkl012mno345pqr678' : '••••••••••••••••••••••••••••••••••••••'}
              </span>
            </div>
            <button onClick={() => setShowKey(v => !v)}
              className="h-10 w-10 rounded-lg border border-slate-700 flex items-center justify-center text-slate-400 hover:bg-slate-700 transition-colors">
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {INTEGRATIONS.map(i => {
          ...
          return (
            <div key={i.id} className="p-4 rounded-2xl border border-slate-700/60 bg-[#1e293b] flex items-start gap-3">
              <span className="text-2xl">{i.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-bold text-slate-200 text-[13px]">{i.name}</span>
    ...
    <div>
      ...
      <div className="rounded-2xl overflow-hidden border border-slate-700/60 bg-[#1e293b]">
        <div className="grid grid-cols-[1fr_100px_100px] gap-0">
          {/* Header */}
          <div className="px-4 py-3 border-b border-slate-700 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Event</div>
          {['Email', 'In-App'].map(h => (
            <div key={h} className="px-4 py-3 border-b border-slate-700 text-[11px] font-semibold text-slate-400 uppercase tracking-wider text-center">{h}</div>
          ))}
          {/* Rows */}
          {NOTIF_PREFS.map(p => (
            <>
              <div key={`${p.id}-label`} className="px-4 py-3 border-b border-slate-700/50 text-[13px] text-slate-200">{p.label}</div>
              {['email', 'in_app'].map(ch => (
                <div key={`${p.id}-${ch}`} className="px-4 py-3 border-b border-slate-700/50 flex items-center justify-center">
                  <button onClick={() => toggle(p.id, ch)}
                    className={`h-5 w-5 rounded border transition-colors flex items-center justify-center ${
                      enabled[p.id]?.has(ch) ? 'bg-indigo-600 border-indigo-600' : 'border-slate-600 bg-slate-800'
                    }`}>
...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-full" style={{ background: '#0f172a' }}>
      <div className="mb-6">
        <h1 className="text-[22px] font-bold text-slate-100">Settings</h1>
        <p className="text-[13px] text-slate-400 mt-0.5">Configure your HR system preferences and integrations</p>
      </div>
      ...
      <div className="flex gap-5">
            ...
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => setActive(s.id)}
                className={`w-full flex items-center gap-2.5 px-3.5 py-3 text-[13px] font-medium text-left transition-colors relative ${
                  active === s.id ? 'text-indigo-400 bg-indigo-600/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-700/30'
                }`}>
                {active === s.id && <span className="absolute left-0 inset-y-2 w-0.5 bg-indigo-500 rounded-r-full" />}
        ...
        {/* Content */}
        <div className="flex-1 min-w-0">
          <div className="rounded-2xl border border-slate-700/60 p-5 bg-[#1e293b]">
            <ActiveSection />
          </div>
        </div>
      </div>
