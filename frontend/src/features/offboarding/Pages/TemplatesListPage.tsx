...
import { useOffboardingTemplates } from '../Hooks/useOffboardingTemplates';

export default function TemplatesListPage() {
  const locale = useLocale();
  const { templates, loading, remove } = useOffboardingTemplates();
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="text-xl font-bold text-brand-text">Offboarding Templates</h1>
          <p className="text-sm text-brand-text-secondary mt-0.5">Task lists, asset checklists & access revocation lists by exit type</p>
        </div>
        <Link href={`/${locale}/offboarding/templates/new`} className="flex items-center gap-2 h-9 px-4 bg-brand-primary hover:bg-brand-primary-hover text-white text-sm font-semibold rounded-lg transition-colors">
          ...
          {templates.map(t => (
            <div key={t._id} className="bg-brand-bg-soft border border-brand-border/60 rounded-2xl p-5 flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-1">
                {t.exitTypes.map(e => (
                  <span key={e} className="text-[10px] px-2 py-0.5 rounded-full bg-brand-primary/15 text-indigo-300 capitalize">{e.replace('_', ' ')}</span>
                ))}
              </div>
              <p className="text-[11px] text-brand-text-muted">
                {t.taskLists.reduce((n, l) => n + l.tasks.length, 0)} tasks · {t.assetChecklist.length} assets · {t.accessRevocationList.length} access items
              </p>
              <div className="flex items-center gap-2 mt-auto pt-2 border-t border-brand-border/60">
                <Link href={`/${locale}/offboarding/templates/${t._id}/edit`} className="flex items-center gap-1.5 text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition-colors">
