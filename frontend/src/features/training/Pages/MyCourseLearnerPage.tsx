...
import { CheckCircle2, Circle, PlayCircle, FileText, Link2, Box, Award, ArrowLeft } from 'lucide-react';
...
function ModuleContentView({ module_ }: { module_: CourseModule }) {
  ...
  if (module_.type === 'video') {
    return c.url ? <video controls src={c.url} className="w-full rounded-lg bg-black max-h-[420px]" /> : <p className="text-sm text-slate-400">No video URL configured.</p>;
  }
  if (module_.type === 'document') {
    return c.fileUrl ? (
      <a href={c.fileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-primary hover:underline text-sm">
