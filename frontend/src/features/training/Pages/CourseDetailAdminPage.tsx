...
import { COURSE_STATUS_STYLES, ENROLLMENT_STATUS_STYLES, ENROLLMENT_STATUS_LABELS, MODULE_TYPE_OPTIONS } from '../constants';
        ...
        <tbody>
          {enrollments.map((e) => (
            <tr key={e._id} className="border-t border-slate-100">
              ...
              <td className="px-4 py-2 text-slate-500">{e.employee?.department || '—'}</td>
              <td className="px-4 py-2 text-slate-600">{e.progressPercentage}%</td>
              <td className="px-4 py-2">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${ENROLLMENT_STATUS_STYLES[e.status]}`}>{ENROLLMENT_STATUS_LABELS[e.status]}</span>
              </td>
              <td className="px-4 py-2 text-slate-500">{e.dueDate ? new Date(e.dueDate).toLocaleDateString() : '—'}</td>
              <td className="px-4 py-2">
          ...
          {enrollments.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">No enrollments yet.</td></tr>}
        </tbody>
      </table>
    </div>
  );
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <div className="space-y-2">
          {data.funnel.map((f) => (
            <div key={f.moduleId}>
              <div className="flex justify-between text-xs text-slate-600 mb-1"><span>{f.title}</span><span>{f.completedCount} completed ({f.dropOffRate}% drop-off)</span></div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2 text-sm">
        ...
        <p className="flex justify-between"><span className="text-slate-500">Total Enrollments</span><span className="font-medium">{data.totalEnrollments}</span></p>
        <p className="flex justify-between"><span className="text-slate-500">Avg Quiz Score</span><span className="font-medium">{data.avgQuizScore != null ? `${data.avgQuizScore}%` : '—'}</span></p>
        <p className="flex justify-between"><span className="text-slate-500">Avg Time to Complete</span><span className="font-medium">{data.avgTimeToCompleteDays != null ? `${data.avgTimeToCompleteDays}d` : '—'}</span></p>
        <div className="pt-2 border-t border-slate-100">
          <p className="text-xs text-slate-500 mb-1">Rating Breakdown</p>
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-100">{course.title}</h1>
          <p className="text-sm text-slate-400">{course.category} · {course.difficultyLevel}</p>
        </div>
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${COURSE_STATUS_STYLES[course.status]}`}>{course.status}</span>
      </div>
