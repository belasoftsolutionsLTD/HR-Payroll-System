...
function PostCard({ post, onReact, currentUserId }: { post: Post; onReact: (id: string, type: string) => void; currentUserId?: string }) {
  ...
  return (
    <div className={cn(
      'bg-brand-bg-soft border border-brand-border rounded-2xl overflow-hidden',
      ...
      <div className="p-5">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-brand-text">{post.author?.fullName ?? 'Someone'}</p>
            <p className="text-[11px] text-brand-text-muted">{post.author?.designation ?? ''} · {timeAgo(post.createdAt)}</p>
          </div>
        </div>

        {/* Content */}
        <p className="text-sm text-brand-text-secondary leading-relaxed whitespace-pre-wrap">{post.content}</p>
          ...
          {reactionCounts.map(({ type, emoji, count, mine }) => (
            <button key={type} onClick={() => onReact(post._id, type)}
              className={cn(
                ...
                mine
                  ? 'bg-brand-primary/20 border-brand-primary/40 text-indigo-300'
                  : 'bg-brand-bg-soft border-brand-border text-brand-text-secondary hover:border-brand-border-strong',
              )}>
            ...
            {comments.map(c => (
              <div key={c._id} className="flex gap-2">
                ...
                <div className="flex-1 bg-brand-bg-soft rounded-xl px-3 py-2">
                  <p className="text-xs font-semibold text-brand-text-secondary">{c.author?.fullName}</p>
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        className="w-full bg-brand-bg-soft border border-brand-border rounded-2xl px-5 py-4 text-left text-sm text-brand-text-muted hover:border-brand-border-strong hover:text-brand-text-muted transition-colors flex items-center gap-3">
        <div className="h-9 w-9 rounded-full bg-brand-primary/20 flex items-center justify-center shrink-0">
    ...
    <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-5 space-y-4">
        ...
        {[
          ...
          <button key={v} onClick={() => setType(v)}
            className={cn(
              ...
              type === v ? 'bg-brand-primary border-brand-primary text-white' : 'border-brand-border text-brand-text-secondary hover:border-brand-border-strong',
            )}>
            {e} {l}
          </button>
        ))}
      ...
      <div className="flex justify-end gap-2">
        <button onClick={() => { setOpen(false); setContent(''); }}
          className="px-4 py-2 text-sm text-brand-text-secondary hover:text-brand-text transition-colors">
          Cancel
        </button>
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="py-16 text-center space-y-2">
          ...
          <p className="text-brand-text-muted text-sm">No posts yet. Be the first to share something!</p>
        </div>
      ) : (
...
function CommunitiesSection() {
  ...
  const TYPE_COLORS: Record<string, string> = {
    company: 'text-blue-400', team: 'text-emerald-400', interest: 'text-purple-400', private: 'text-brand-text-secondary',
  };

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-brand-text">Communities</h2>
      ...
      {showCreate && (
        <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-brand-text">New Community</h3>
          <div className="grid grid-cols-[60px_1fr] gap-3">
            <input value={newIcon} onChange={e => setNewIcon(e.target.value)}
              className="h-10 bg-brand-bg-soft border border-brand-border rounded-xl px-3 text-center text-xl focus:outline-none focus:border-brand-primary" />
            <input value={newName} onChange={e => setNewName(e.target.value)}
              placeholder="Community name *"
              className="h-10 bg-brand-bg-soft border border-brand-border rounded-xl px-3 text-sm text-brand-text placeholder:text-brand-text-muted focus:outline-none focus:border-brand-primary" />
          </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {communities.map(c => (
            <div key={c._id} className="bg-brand-bg-soft border border-brand-border rounded-2xl p-5 hover:border-brand-border-strong transition-colors">
              <div className="flex items-start gap-3 mb-3">
                <div className="h-12 w-12 rounded-2xl bg-brand-bg-soft flex items-center justify-center text-2xl shrink-0">
                  {c.icon || '👥'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-brand-text truncate">{c.name}</p>
                  <p className={cn('text-[10px] font-semibold uppercase tracking-wide capitalize', TYPE_COLORS[c.type] || 'text-brand-text-muted')}>
                    {c.type}
                  </p>
                </div>
              </div>
              {c.description && <p className="text-xs text-brand-text-muted mb-3 line-clamp-2">{c.description}</p>}
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-brand-text-muted">{c.memberCount} members</span>
                <button onClick={() => join(c._id, c.isMember)}
                  className={cn(
                    'px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors',
                    c.isMember
                      ? 'bg-brand-bg-muted text-brand-text-secondary hover:bg-red-500/10 hover:text-red-400'
...
function CelebrationSection() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-brand-text">Celebrations</h2>
        <p className="text-xs text-brand-text-muted mt-0.5">Upcoming milestones in the next 30 days</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-brand-text-muted text-sm">No upcoming celebrations in the next 30 days.</p>
        </div>
      ) : (
      ...
      {clapping && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md bg-white border border-brand-border rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-brand-text">Send a Clap 👏</h3>
              <button onClick={() => setClapping(null)} className="text-brand-text-muted hover:text-brand-text-secondary">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="text-center py-2">
              <div className="text-5xl mb-2">{CELEBRATION_META[clapping.type].emoji}</div>
              <p className="text-sm font-bold text-brand-text">{clapping.employee.fullName}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button key={t} onClick={() => setMsg(t)}
                  className="text-xs px-3 py-1.5 rounded-full bg-brand-bg-soft border border-brand-border text-brand-text-secondary hover:border-brand-primary hover:text-indigo-400 transition-colors">
                  {t}
                </button>
              ))}
            </div>
...
function MeetingsSection() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-brand-text">1:1 Meetings</h2>
          <p className="text-xs text-brand-text-muted mt-0.5">Your recurring 1:1 meeting series</p>
        </div>
      ...
      {showSchedule && (
        <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-brand-text">Schedule a 1:1</h3>

          {/* Selected people chips */}
          {selectedPeople.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {selectedPeople.map(p => (
                <span key={p._id} className="flex items-center gap-1.5 bg-brand-primary/20 border border-brand-primary/30 text-indigo-300 text-xs font-semibold px-2.5 py-1 rounded-full">
                  {p.fullName}
                  <button type="button" onClick={() => removePerson(p._id)} className="hover:text-brand-text transition-colors">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              ))}
          ...
          <div className="relative">
            ...
            />
            {empResults.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-brand-border rounded-xl shadow-2xl z-20 overflow-hidden">
                {empResults
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[11px] text-brand-text-muted block mb-1">Frequency</label>
              <select value={form.frequency} onChange={e => setForm(f => ({ ...f, frequency: e.target.value }))}
                className="w-full h-9 bg-brand-bg-soft border border-brand-border rounded-lg px-2 text-xs text-brand-text focus:outline-none focus:border-brand-primary">
            ...
            <div>
              <label className="text-[11px] text-brand-text-muted block mb-1">Time</label>
              <input type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))}
                className="w-full h-9 bg-brand-bg-soft border border-brand-border rounded-lg px-2 text-xs text-brand-text focus:outline-none focus:border-brand-primary" />
            </div>
            <div>
              <label className="text-[11px] text-brand-text-muted block mb-1">Duration</label>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowSchedule(false); setSelectedPeople([]); }} className="px-4 py-2 text-sm text-brand-text-secondary">Cancel</button>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-brand-text-muted text-sm">No 1:1 meetings yet. Schedule your first one!</p>
        </div>
      ) : (
        <div className="space-y-3">
          {series.map(s => (
            <div key={s._id} className="bg-brand-bg-soft border border-brand-border rounded-xl p-4 flex items-center gap-4 hover:border-brand-border-strong transition-colors">
              <Avatar name={s.otherParticipant?.fullName} size="lg" color="#6366f1" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-brand-text">{s.otherParticipant?.fullName ?? 'Participant'}</p>
                <p className="text-xs text-brand-text-muted">{s.otherParticipant?.designation}</p>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {s.time && <span className="flex items-center gap-1 text-[11px] text-brand-text-muted"><Clock className="h-3 w-3" /> {s.time}</span>}
                </div>
              </div>
              {s.lastMeeting && (
                <div className="text-right shrink-0">
                  <p className="text-[10px] text-brand-text-muted">Last meeting</p>
                  <p className="text-xs text-brand-text-secondary">{new Date(s.lastMeeting.date).toLocaleDateString('en-KE', { dateStyle: 'medium' })}</p>
                </div>
              )}
            </div>
          ))}
...
function TrustChannelSection() {
  ...
  const REPORT_STATUS_MAP: Record<string, Status> = {
    new: 'new', under_review: 'pending', resolved: 'completed', closed: 'closed',
  };

  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <div className="h-14 w-14 rounded-2xl bg-brand-primary/10 flex items-center justify-center mx-auto">
          <Shield className="h-7 w-7 text-indigo-400" />
        </div>
        <h2 className="text-lg font-bold text-brand-text">Trust Channel</h2>
        <p className="text-sm text-brand-text-muted">Speak up safely. Reports are completely anonymous.</p>
      </div>

      <div className="flex bg-brand-bg-soft rounded-xl p-1">
        {([
          ...
          <button key={key} onClick={() => setTab(key)}
            className={cn('flex-1 py-2 rounded-lg text-xs font-semibold capitalize transition-colors',
              tab === key ? 'bg-brand-bg-muted text-brand-text' : 'text-brand-text-muted hover:text-brand-text-secondary')}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === 'submit' && !submitted && (
        <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-6 space-y-5">
          <div className="space-y-2">
            <label className="text-xs font-semibold text-brand-text-secondary uppercase tracking-wide">Category *</label>
            {TRUST_CATEGORIES.map(cat => (
              <label key={cat} className="flex items-center gap-2.5 cursor-pointer">
                <div className={cn('h-4 w-4 rounded-full border-2 transition-colors shrink-0',
                  category === cat ? 'border-brand-primary bg-brand-primary' : 'border-brand-border-strong')}>
                ...
                <span className="text-sm text-brand-text-secondary">{cat}</span>
              </label>
            ))}
          </div>
          <div>
            <label className="text-xs font-semibold text-brand-text-secondary uppercase tracking-wide block mb-2">Description *</label>
            <textarea value={desc} onChange={e => setDesc(e.target.value)}
              rows={5} placeholder="Be as specific as possible. Do not include your own name. (min 20 characters)"
              className="w-full bg-brand-bg-soft border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-text placeholder:text-brand-text-muted focus:outline-none focus:border-brand-primary resize-none" />
            <p className="text-right text-[10px] text-brand-text-muted mt-1">{desc.length}/500</p>
          </div>
          <button onClick={submit} disabled={submitting || !category || desc.length < 20}
            className="w-full py-3 rounded-xl bg-brand-primary hover:bg-brand-primary-hover text-white text-sm font-semibold disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
      ...
      {tab === 'submit' && submitted && (
        <div className="bg-brand-bg-soft border border-emerald-500/30 rounded-2xl p-6 text-center space-y-4">
          <div className="h-12 w-12 rounded-full bg-emerald-500/10 flex items-center justify-center mx-auto">
            <Check className="h-6 w-6 text-emerald-400" />
          </div>
          <div>
            <p className="text-base font-bold text-brand-text">Report Submitted</p>
            <p className="text-sm text-brand-text-muted mt-1">Your report has been submitted anonymously.</p>
          </div>
          <div className="bg-brand-bg-soft rounded-xl p-4">
            <p className="text-xs text-brand-text-muted mb-1">Your tracking code</p>
            <p className="text-2xl font-black text-indigo-400 tracking-widest">{code}</p>
            <p className="text-xs text-brand-text-muted mt-1">Save this code to check for updates.</p>
          </div>
      ...
      {tab === 'check' && (
        <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-6 space-y-4">
          <p className="text-sm text-brand-text-secondary">Enter your 8-character tracking code to check the status of your report.</p>
          <div className="flex gap-2">
            <input value={checkCode} onChange={e => setCheckCode(e.target.value)}
              placeholder="XXXX-XXXX"
              className="flex-1 h-10 bg-brand-bg-soft border border-brand-border rounded-xl px-3 text-sm text-brand-text placeholder:text-brand-text-muted focus:outline-none focus:border-brand-primary tracking-widest uppercase" />
            <button onClick={check} disabled={checking || !checkCode.trim()}
              className="px-4 py-2 rounded-xl bg-brand-primary hover:bg-brand-primary-hover text-white text-sm font-semibold disabled:opacity-50 transition-colors">
              {checking ? '…' : 'Check'}
            </button>
          </div>
          {checkResult && (
            <div className="bg-brand-bg-soft rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs text-brand-text-muted capitalize">{checkResult.category}</p>
                <StatusBadge status={REPORT_STATUS_MAP[checkResult.status] ?? 'new'} label={checkResult.status.replace('_', ' ')} className="text-xs font-bold py-1" />
              </div>
              <p className="text-xs text-brand-text-muted">Last updated: {new Date(checkResult.updatedAt).toLocaleDateString('en-KE', { dateStyle: 'medium' })}</p>
              {checkResult.responseToReporter ? (
                <div className="border border-brand-primary/30 bg-brand-primary/5 rounded-lg px-3 py-2.5">
                  <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-wide mb-1">Response from HR</p>
                  <p className="text-xs text-brand-text-secondary leading-relaxed">{checkResult.responseToReporter}</p>
                </div>
              ) : (
                <p className="text-xs text-brand-text-muted italic">No response yet. Check back later.</p>
              )}
            </div>
          )}
        </div>
      ...
      {tab === 'admin' && isHR && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs text-brand-text-muted">{reports.length} report{reports.length !== 1 ? 's' : ''} total</p>
          ...
          {loadingReports ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-brand-text-muted text-sm">No trust reports yet.</p>
            </div>
          ) : reports.map(r => (
            <div key={r._id} className="bg-brand-bg-soft border border-brand-border rounded-xl overflow-hidden">
              <div className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-brand-bg-soft/40 transition-colors"
                onClick={() => setExpandedReport(expandedReport === r._id ? null : r._id)}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold text-brand-text capitalize">{r.category}</p>
                    <StatusBadge status={REPORT_STATUS_MAP[r.status] ?? 'new'} label={r.status.replace('_', ' ')} className="text-[10px] font-bold" />
                  </div>
                  <p className="text-[11px] text-brand-text-muted mt-0.5">
                    Code: <span className="font-mono text-brand-text-secondary">{r.trackingCode}</span>
                    {' · '}{new Date(r.createdAt).toLocaleDateString('en-KE', { dateStyle: 'medium' })}
                  </p>
                </div>
                <span className="text-brand-text-muted text-xs">{expandedReport === r._id ? '▲' : '▼'}</span>
              </div>
              {expandedReport === r._id && (
                <div className="border-t border-brand-border px-5 py-4 space-y-4 bg-brand-bg-soft/20">
                  <p className="text-sm text-brand-text-secondary leading-relaxed">{r.description}</p>
                  {r.responseToReporter && (
                    <div className="border border-brand-primary/20 bg-brand-primary/5 rounded-lg px-3 py-2">
                      <p className="text-[10px] text-indigo-400 font-semibold uppercase mb-1">Sent to Reporter</p>
                      <p className="text-xs text-brand-text-secondary">{r.responseToReporter}</p>
                    </div>
                  )}
                  {r.adminNotes && (
                    <div className="bg-brand-bg-soft rounded-lg px-3 py-2">
                      <p className="text-[10px] text-brand-text-muted font-semibold uppercase mb-1">Internal Notes</p>
                      <p className="text-xs text-brand-text-secondary">{r.adminNotes}</p>
                    </div>
                  )}
...
function AdminReportActions({ report, onUpdate }: { report: any; onUpdate: (id: string, status: string, notes: string, response: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="text-[10px] text-brand-text-muted font-semibold uppercase block mb-1">Update Status</label>
      ...
      <div>
        <label className="text-[10px] text-brand-text-muted font-semibold uppercase block mb-1">
        ...
        <textarea value={response} onChange={e => setResponse(e.target.value)}
          ...
          className="w-full bg-brand-bg-soft border border-brand-border rounded-lg px-3 py-2 text-xs text-brand-text placeholder:text-brand-text-muted focus:outline-none focus:border-brand-primary resize-none" />
      </div>
      <div>
        <label className="text-[10px] text-brand-text-muted font-semibold uppercase block mb-1">
          Internal Notes <span className="text-brand-text-muted normal-case font-normal">(HR only — not shown to reporter)</span>
        </label>
      ...
      <button onClick={() => onUpdate(report._id, status, notes, response)}
        className="px-4 py-2 rounded-lg bg-brand-primary hover:bg-brand-primary-hover text-white text-xs font-semibold transition-colors">
        Save Changes
      </button>
    </div>
  );
...
function AnnouncementsSection() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-brand-text">Announcements</h2>
        {isHR && (
          <button onClick={() => setShowCreate(v => !v)}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-primary hover:bg-brand-primary-hover text-white text-sm font-semibold transition-colors">
      ...
      {showCreate && (
        <div className="bg-brand-bg-soft border border-brand-border rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-brand-text">Create Announcement</h3>
          <div className="flex gap-2">
            {[{ v: 'news', l: '📰 News' }, { v: 'alert', l: '🚨 Alert' }, { v: 'campaign', l: '📣 Campaign' }].map(({ v, l }) => (
              <button key={v} onClick={() => setForm(f => ({ ...f, type: v }))}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all',
                  form.type === v ? 'bg-brand-primary border-brand-primary text-white' : 'border-brand-border text-brand-text-secondary hover:border-brand-border-strong')}>
                {l}
              </button>
            ))}
          </div>
          ...
          <textarea value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))}
            ...
            className="w-full bg-brand-bg-soft border border-brand-border rounded-xl px-3 py-2 text-sm text-brand-text placeholder:text-brand-text-muted focus:outline-none focus:border-brand-primary resize-none" />
          <div>
            <p className="text-xs font-semibold text-brand-text-secondary uppercase tracking-wide mb-2">Send To</p>
            <div className="flex gap-2 flex-wrap">
              {AUDIENCE_OPTS.map(({ value, label }) => (
                <button key={value} onClick={() => toggleAudience(value)}
                  className={cn('px-3 py-1.5 rounded-full text-xs font-semibold border transition-all',
                    form.audiences.includes(value) ? 'bg-brand-primary/20 border-brand-primary text-indigo-300' : 'border-brand-border text-brand-text-muted hover:border-brand-border-strong')}>
                  {label}
                </button>
              ))}
            </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-brand-text-muted text-sm">No announcements yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {announcements.map(a => (
            <div key={a._id} className="bg-brand-bg-soft border border-brand-border rounded-xl p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-brand-text-secondary leading-relaxed">{a.body}</p>
...
export default function CommunicationPage() {
  ...
  return (
    <div className="flex h-full min-h-screen bg-white">
      {/* Left sidebar */}
      <aside className="w-56 shrink-0 border-r border-brand-border py-6 px-3 flex flex-col gap-1">
        <p className="text-[10px] font-bold text-brand-text-muted uppercase tracking-widest px-3 mb-3">Communication</p>
        {NAV_ITEMS.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setSection(key)}
            className={cn(
              'flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-all w-full text-left',
              section === key
                ? 'bg-brand-primary/15 text-indigo-400 border border-brand-primary/20'
